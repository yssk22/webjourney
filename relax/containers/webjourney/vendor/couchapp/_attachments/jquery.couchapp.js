/**
 * modified by yssk22 for WebJourney environment.
 * Add docForm Features:
 * - add failure callback for docForm
 * - cancel posting when beforeSave callback returns false.
 * - support checked fields on deep-JSON serialization.
 *
 */
         ...
         function docForm(formSelector, opts) {
            ...
            function formToDeepJSON(form, fields, doc) {
               ...
               opts.fields.forEach(function(field) {
                  ...
                  case "radio":
                  case "checkbox":
                     val = form.find("[name="+field+"]:checked").val();
                     break;
                  default:
                     val = elm.val();
                     break;
            ...
            $(formSelector).submit(function(e) {
               ...
               if (opts.beforeSave){
                  cancel = !opts.beforeSave(localFormDoc);
               };
               if(!cancel){
                  db.saveDoc(localFormDoc, {
                     ...
                     failure : function(resp){
                        if (opts.failure) opts.failure(resp, localFormDoc);
                     }
                  });
               }
